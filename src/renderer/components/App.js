        ...
        <Router><ScrollContext>
          <Switch>
            <Route path="/player/:cloc/:vid" render={({match, history}) => {
              ...
              return <Player video={mainState.collections.get(collectionLocator).videos.get(videoId)} onExit={() => {
                history.goBack();
                ...
                return actions.loadVideoPlaybackPosition(collectionLocator, videoId);
                ...
                actions.setPreference(pref, value);
                             ...
                             searchDictionaries={actions.searchDictionaries} getWordFromList={actions.getWordFromList}
                             setWordInList={actions.setWordInList}
                             onExtractAudio={(startTime, endTime) => extractAudioFromVideo(collectionLocator, videoId, startTime, endTime)}
                             onExtractFrameImage={(time) => extractFrameImageFromVideo(collectionLocator, videoId, time)}
                          ...
                          {title.parts.episodes.length ? (
                            <ul>
                              {title.parts.episodes.map(ep => (
                                <VideoListItem collection={collection} videoId={ep.videoId} name={'Episode ' + ep.episodeNumber}
                          ...
                          {title.parts.others.length ? (
                            <ul>
                              {title.parts.others.map(other => (
                                <VideoListItem collection={collection} videoId={other.videoId} name={other.name}
                                               playbackPosition={other.playbackPosition} key={other.name}/>
                              ))}
                            </ul>
                          ) : null}
                        </div>
                          ...
                          <li className="App-collection" key={collection.locator}>
                            ...
                            <ul>
                              {collection.titles.map(title => title.series ? (
                                  ...
                                  <VideoListItem collection={collection} videoId={title.videoId} name={title.name}
                                                 playbackPosition={title.playbackPosition} key={title.name}/>
                                )
                              )}
                            </ul>
                          </li>
